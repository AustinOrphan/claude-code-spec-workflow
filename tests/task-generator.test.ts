...
`;
    ...
    test('should handle various format variations agents might produce', () => {
      // Test without period after number
      const noPeriodContent = `- [ ] 1 Task without period
  - Some details
  - _Requirements: 1.1_
...
- [ ] 2 Another task
  - Details here`;
      ...
      const noPeriodTasks = parseTasksFromMarkdown(noPeriodContent);
      expect(noPeriodTasks).toHaveLength(2);
      expect(noPeriodTasks[0].id).toBe('1');
      expect(noPeriodTasks[0].description).toBe('Task without period');
      expect(noPeriodTasks[0].requirements).toBe('1.1');
      
      // Test with extra spaces
      const extraSpacesContent = `- [ ]   1.   Task with extra spaces  
  - Details
  - _Requirements: 1.1_

-  [ ]  2.1  Subtask with spaces`;
      
      const extraSpacesTasks = parseTasksFromMarkdown(extraSpacesContent);
      expect(extraSpacesTasks).toHaveLength(2);
      expect(extraSpacesTasks[0].id).toBe('1');
      expect(extraSpacesTasks[0].description).toBe('Task with extra spaces');
      expect(extraSpacesTasks[1].id).toBe('2.1');
      expect(extraSpacesTasks[1].description).toBe('Subtask with spaces');
      
      // Test with requirements/leverage inline
      const inlineMetadataContent = `- [ ] 1. Task with inline metadata
  - Implementation detail here _Requirements: 1.1_ and more text
  - Another detail with _Leverage: existing.ts_ in the middle`;
      
      const inlineMetadataTasks = parseTasksFromMarkdown(inlineMetadataContent);
      expect(inlineMetadataTasks).toHaveLength(1);
      expect(inlineMetadataTasks[0].requirements).toBe('1.1');
      expect(inlineMetadataTasks[0].leverage).toBe('existing.ts');
    });

    test('should handle deeply nested task IDs', () => {
      const deeplyNestedContent = `- [ ] 1. Main task
  - _Requirements: 1.0_

- [ ] 1.1 First level subtask
  - _Requirements: 1.1_

- [ ] 1.1.1 Second level subtask
  - _Requirements: 1.1.1_

- [ ] 1.1.1.1 Third level subtask
  - Details for deep task
  - _Requirements: 1.1.1.1_
  - _Leverage: deep/component.ts_`;
      
      const deepTasks = parseTasksFromMarkdown(deeplyNestedContent);
      expect(deepTasks).toHaveLength(4);
      expect(deepTasks[3].id).toBe('1.1.1.1');
      expect(deepTasks[3].description).toBe('Third level subtask');
      expect(deepTasks[3].requirements).toBe('1.1.1.1');
      expect(deepTasks[3].leverage).toBe('deep/component.ts');
    });

    test('should handle mixed formats in same document', () => {
      const mixedFormatsContent = `# Implementation Plan

- [ ] 1. Task with period
  - _Requirements: 1.1_

- [ ] 2 Task without period
  - Details here

- [ ]  3.1.  Subtask with extra spaces and periods  
  - More details
  - _Leverage: component.ts_

- [ ] 4. Task with everything
  - Detail line with _Requirements: 4.1_ inline
  - Another line with _Leverage: utils.js_ also inline
  - More details`;
      
      const mixedTasks = parseTasksFromMarkdown(mixedFormatsContent);
      expect(mixedTasks).toHaveLength(4);
      
      expect(mixedTasks[0].description).toBe('Task with period');
      expect(mixedTasks[1].description).toBe('Task without period');
      expect(mixedTasks[2].description).toBe('Subtask with extra spaces and periods');
      expect(mixedTasks[3].description).toBe('Task with everything');
      expect(mixedTasks[3].requirements).toBe('4.1');
      expect(mixedTasks[3].leverage).toBe('utils.js');
    });
